<template>
  ...
  >
    <div class="gift-content">
      ...
      <div
        class="content"
        v-else-if="type === 'get-gift'"
      >
        <div
          class="icon"
          :style="{backgroundImage:`url(${require(`@imgs/icon_${type}.png`)})`}"
        ></div>
        <div
          :class="['text' , {'hasGain':!hasGain || cantGain}]"
          v-if="giftInfo.length >= 1"
        >
          <template v-if="cantGain">奖品是{{giftInfo[giftIndex] ? giftInfo[giftIndex].giftName:giftInfo[0].giftName}}</template>
          <template v-else>
        ...
        <div
          class="btn"
          v-if="hasGain && !cantGain"
          @click="recieve"
        >{{btnText[type]}}</div>
      </div>
      <div
        class="content"
        v-else-if="type === 'sign-success'"
      >
        ...
        <template v-else>
          <div
            class="icon"
            :style="{backgroundImage:`url(${require(`@imgs/icon_mark.png`)})`}"
          ></div>
            ...
            >
              ...
              >
                <div class="text">
                  <div
                    class="taobao-text"
                    v-if="item.days && !hasGain"
                  >累计签到 {{item.days}} 天可获得</div>
