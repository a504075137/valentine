...
export default {
  ...
  methods: {
    init() {
      ...
      this.$bus.continueDay =
        this.$bus.signInfo.markList.length > 0
          ? this.$bus.signInfo.markList[0].continueDays
          : 0;
      this.getGiftDate();
      this.judgeShare();
      this.$bus.$on("goRule", () => {
    ...
    async mark() {
      console.log(897515, this.$bus.isLogin);
      if (!this.$bus.isLogin) {
      ...
      this.$loading.show();
      const markResult = await this.$api.mark({
      ...
      if (sendGiftList.length === hasList.length) {
        if (markResult.sendGiftList.length > 0) {
          ...
          this.$dialog.show("gift", {
            vBind: {
              type: "display",
              hasGain: true,
              giftInfo: markResult.sendGiftList
            }
          });
        } else {
          this.$dialog.show("gift", { vBind: { type: "sign-success" } });
        ...
        this.$dialog.show("gift", {
          vBind: {
            type: "display",
            hasGain: true,
            giftInfo: [...sendGiftList, ...noneGift]
          }
        });
      }
    },
